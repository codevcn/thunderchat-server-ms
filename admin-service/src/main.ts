...
async function bootstrap() {
  await beforeLaunch()
  console.log('>>> Microservice [Admin-Service] is launching...')
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      package: EGrpcPackages.ADMIN,
      protoPath: join(__dirname, '/../protos/artifacts/', 'admin.proto'),
      url: `${HOST_ADDRESS}:${GRPC_PORT}`,
    },
  })

  await app.startAllMicroservices()
  console.log(`>>> Microservice [Admin-Service] is listening on: ${HOST_ADDRESS}:${GRPC_PORT}`)

  await app.listen(PORT, HOST_ADDRESS)
  console.log(`>>> HTTP Server [Admin-Service] is listening on: ${HOST_ADDRESS}:${PORT}`)
}
bootstrap()
