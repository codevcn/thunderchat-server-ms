...
export class UserService {
  ...
  private static inMemoryHashStore: Map<string, Map<string, string>> =
    new Map();
  ...
  async requestPasswordReset(email: string) {
    ...
    if (
      existing &&
      existing.lastSentAt &&
      now - Number(existing.lastSentAt) < 60 * 1000
    ) {
      return;
    }
  ...
  async verifyPasswordResetOtp(
    ...
    await this.storeSetEx(
      tokenKey,
      Math.ceil(this.getResetTokenTTLms() / 1000),
      resetToken,
    );
    await this.storeDel(otpKey);
    if (
  ...
  async createUser({
    ...
    fullName,
    birthday,
  }: TCreateUserParams): Promise<TUser> {
    const hashedPassword =
      await this.credentialService.getHashedPassword(password);
