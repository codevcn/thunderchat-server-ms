...
export class UserReportService implements IUserReportService {
  ...
  //             const { url: imageUrl } =
  //               await this.uploadService.uploadReportImage(image);
  ...
  //             }
  //             throw new Error(
  //               `${EUserReportMessages.UPLOAD_FAILED}: ${error.message}`,
  //             );
  //           }
  //         }
  //       }
  ...
  //                 if (!url || url.trim() === '') {
  //                   throw new Error(
  //                     `AWS upload failed: No URL returned for message ${messageId}`,
  //                   );
  //                 }
  ...
  //                 throw new Error(
  //                   `Media upload failed for message ${messageId}: ${error.message}`,
  //                 );
  //               }
  ...
  //                 throw new Error(
  //                   `${EUserReportMessages.FILE_NOT_FOUND}: ${messageContent}`,
  //                 );
  //               }
  ...
  //                 // Add timeout for upload operation
  //                 const uploadPromise =
  //                   this.uploadService.uploadReportMessageMedia(
  //                     messageContent,
  //                     messageId,
  //                     this.getContentTypeForMessageType(messageType),
  //                   );
  ...
  //                           reject(
  //                             new Error(
  //                               `Upload timeout for message ${messageId}`,
  //                             ),
  //                           ),
  //                         60000,
  //                       ), // 60 seconds timeout
  ...
  //               } catch (error) {
  //                 DevLogger.logError(
  //                   'Failed to upload reported message media:',
  //                   error,
  //                 );
  //                 failedUploads.push({ messageId, error: error.message });
  ...
  private getContentTypeForMessageType(
  ...
  private async rollbackUploadedFiles(uploadedUrls: string[]): Promise<void> {
    ...
    try {
      await Promise.race([
        ...
        new Promise(
          (_, reject) =>
            setTimeout(() => reject(new Error('Rollback timeout')), 30000), // 30 seconds timeout for rollback
        ),
      ]);
      DevLogger.logError(
        `Successfully completed rollback for ${uploadedUrls.length} files`,
      );
    } catch (error) {
      ...
      for (const url of uploadedUrls) {
        try {
          ...
          DevLogger.logError(
            `Failed to rollback file after timeout ${url}:`,
            rollbackError,
          );
        }
      }
