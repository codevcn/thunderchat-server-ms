...
export class GroupMemberService {
  ...
  async removeGroupChatMember(
    ...
    if (totalMembers) {
      if (totalMembers < this.MIN_MEMBER_IN_GROUP_CHAT + 1) {
        throw new BadRequestException(
          EGroupMemberMessages.MIN_MEMBER_IN_GROUP_CHAT,
        );
      }
    }
    try {
      ...
      throw new InternalServerErrorException(
        EGroupMemberMessages.FAILED_TO_REMOVE_MEMBER,
      );
    }
    ...
    this.eventEmitter.emit(
      EInternalEvents.REMOVE_GROUP_CHAT_MEMBERS,
      groupChat,
      [memberId],
    );
  }

  async getGroupChatMember(
    groupChatId: number,
    userId: number,
  ): Promise<TGroupChatMember | null> {
    return await this.prismaService.groupChatMember.findFirst({
      where: { groupChatId, userId },
    });
  }

  async checkIfMembersInGroupChat(
    groupChatId: number,
    memberIds: number[],
  ): Promise<boolean> {
  ...
  async addMembersToGroupChat(
    ...
    if (memberIds.length > this.MAX_ADD_MEMBER_AT_ONCE) {
      throw new BadRequestException(
        EGroupMemberMessages.MAX_ADD_MEMBER_AT_ONCE,
      );
    }
    const totalMembers = await this.prismaService.groupChatMember.count({
      where: { groupChatId },
    ...
    const isMembersInGroupChat = await this.checkIfMembersInGroupChat(
      groupChatId,
      memberIds,
    );
    if (isMembersInGroupChat) {
      throw new BadRequestException(
        EGroupMemberMessages.MEMBERS_ALREADY_IN_GROUP_CHAT,
      );
    }
  ...
  async leaveGroupChat(groupChatId: number, userId: number): Promise<void> {
    ...
    if (groupChat && groupChat.creatorId === userId)
      throw new BadRequestException(
        EGroupMemberMessages.ADMIN_CANNOT_LEAVE_GROUP_CHAT,
      );
    await this.prismaService.groupChatMember.delete({
      where: { groupChatId_userId: { groupChatId, userId } },
    });
    this.eventEmitter.emit(
      EInternalEvents.MEMBER_LEAVE_GROUP_CHAT,
      groupChatId,
      userId,
    );
  }
