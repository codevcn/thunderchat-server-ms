...
export class GroupChatRoleGuard implements CanActivate {
  ...
  private async authorizeUser(context: ExecutionContext): Promise<void> {
    ...
    const groupChatId =
      request.query.groupChatId ||
      request.body.groupChatId ||
      request.params.groupChatId;
    if (!groupChatId)
    ...
    const member = await this.groupMemberService.getGroupChatMember(
      groupChatIdNumber,
      user.id,
    );
    if (!member)
      throw new BaseHttpException(
        EGroupChatMessages.MEMBER_NOT_FOUND,
