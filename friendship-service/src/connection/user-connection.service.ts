...
import {
  createDirectChatRoomName,
  createGroupChatRoomName,
} from '@/utils/helpers';
...
export class UserConnectionService {
  ...
  private readonly userChattingConnections = new Map<
    TUserId,
    TDirectChat['id'][]
  >();
  ...
  printOutSession() {
    for (const [key, value] of this.connectedClients) {
      for (const client of value) {
        DevLogger.logInfo(
          `key: ${key} - something: ${client.handshake?.auth.clientId}`,
        );
      }
    }
  }
  ...
  setUserChattingConnection(
    ...
    if (userConnections) {
      if (!userConnections.includes(directChatId))
        userConnections.push(directChatId);
    } else {
    ...
    if (otherUserConnections) {
      if (!otherUserConnections.includes(directChatId))
        otherUserConnections.push(directChatId);
    } else {
